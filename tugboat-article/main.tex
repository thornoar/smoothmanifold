...
is stored in a structure called \texttt{picture} (in particular, the
...
will immediately draw the path, defeating the point.

And indeed, mutating a drawn path can be useful. For example, consider
...
\end{verbatim}
The \texttt{fitpath} function allows the user to draw a path on a picture
and then mutate it afterwards, for example, split it into two subpaths
and draw each one with different pens, as in Figure~\ref{fig:overlap}.
It also allows to leave ``gaps'' in paths after they are drawn, as
seen in Figure~\ref{fig:gaps}, which was generated using this code:
\begin{verbatim}
...
intersection points, taking subpaths, and finally drawing the subpath.
